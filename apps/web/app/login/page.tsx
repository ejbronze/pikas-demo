...
export default function Login() {
  ...
  return <main className="grid min-h-screen bg-pikas-navy lg:grid-cols-2">
    ...
    <section className="grid place-items-center bg-slate-50 p-4"><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl sm:p-8">
      ...
      {!student&&<Link href="/forgot-password" className="mt-5 block text-center font-bold text-blue-700">¿Olvidaste tu contraseña?</Link>}<p className="mt-6 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900">Acceso de demostración con datos ficticios. No se mueve dinero real.</p><p className="mt-5 text-center text-xs font-semibold text-slate-500">PIKAS es diseñada y desarrollada por Palmchat Innovations LLC.</p>
    </div></section>
  </main>;
}
