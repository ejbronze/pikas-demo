...
test("POS separates cash from wallet and preserves its cart",async({page})=>{
  ...
  await page.getByRole("button",{name:"Venta en efectivo"}).click();
  ...
  await page.reload();await expect(page.getByRole("heading",{name:"Carrito persistente"}).locator(".." )).toContainText("Pasta con pollo");
  await page.getByRole("button",{name:"Completar venta en efectivo"}).click();await expect(page.getByRole("status")).toContainText("Venta en efectivo");
  await expect(page.getByRole("heading",{name:"Historial POS"}).locator(".." )).toContainText("Efectivo");
});
