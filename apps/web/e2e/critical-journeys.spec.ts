...
test("Palmchat Innovations attribution is visible on public access surfaces",async({page})=>{for(const route of ["/","/login","/admin/login"]){await page.goto(route);await expect(page.getByText(/diseñada y desarrollada por Palmchat Innovations LLC/i)).toBeVisible()}});
...
test("cafeteria completes a shared, restricted purchase",async({page})=>{await page.goto("/login");const tab=page.getByRole("tab",{name:"Cafetería"});await tab.click();await expect(tab).toHaveAttribute("aria-selected","true");await page.getByRole("button",{name:/Entrar como cafetería/}).click();await expect(page).toHaveURL(/\/pos$/);await page.getByLabel("Código estudiantil / NFC",{exact:true}).fill("PK-00000");await page.getByRole("button",{name:"Comprobar estudiante"}).click();await expect(page.locator("#lookup-error")).toContainText("No encontramos");await page.getByLabel("Código estudiantil / NFC",{exact:true}).fill("PK-10982");await page.getByRole("button",{name:"Comprobar estudiante"}).click();await expect(page.getByRole("heading",{name:"Sofi"})).toBeVisible();const pizza=page.getByRole("article").filter({hasText:"Pizza escolar"});await expect(pizza.getByRole("button")).toBeDisabled();await expect(pizza).toContainText("alergia registrada");const energy=page.getByRole("article").filter({hasText:"Bebidas energéticas"});await expect(energy.getByRole("button")).toBeDisabled();const pasta=page.getByRole("article").filter({hasText:"Pasta con pollo"});await pasta.getByRole("button",{name:"Añadir al carrito"}).click();await page.getByRole("button",{name:"Confirmar venta"}).click();await expect(page.getByRole("heading",{name:"Compra completada"})).toBeVisible();await expect(page.getByRole("heading",{name:"Historial POS"})).toBeVisible();await page.reload();await expect(page.getByText("Pasta con pollo")).toBeVisible();await page.getByRole("button",{name:/Salir|Cerrar sesión/}).click();await page.goto("/login");await page.getByRole("tab",{name:"Estudiante"}).click();await page.getByRole("button",{name:/Entrar como estudiante/}).click();await page.goto("/estudiante/transacciones");await expect(page.getByText("1× Pasta con pollo")).toBeVisible();await page.goto("/api/auth/logout");await page.goto("/login");await page.getByRole("button",{name:/Entrar como familia/}).click();await page.goto("/familias/transacciones");await expect(page.getByText("1× Pasta con pollo")).toBeVisible()});

async function adminLogin(page:import("@playwright/test").Page,email:string){await page.goto("/admin/login");await page.getByLabel("Correo demo").first().fill(email);await page.getByRole("button",{name:"Entrar a administración"}).first().click()}
...
test("POS separates cash from wallet and preserves its cart",async({page})=>{
  ...
  await page.getByRole("button",{name:"Ventas en efectivo"}).click();
  ...
  await page.reload();await page.getByLabel("Código estudiantil / NFC",{exact:true}).fill("PK-10982");await page.getByRole("button",{name:"Comprobar estudiante"}).click();await expect(page.getByRole("heading",{name:"Carrito persistente"}).locator(".." )).toContainText("Pasta con pollo");
  await page.getByLabel("Efectivo recibido (RD$)").fill("200");await expect(page.getByLabel("Efectivo recibido (RD$)")).toHaveValue("200");await page.getByRole("button",{name:"Confirmar venta"}).click();await expect(page.getByRole("status")).toContainText("Cash — Student-linked");
  await expect(page.getByRole("heading",{name:"Historial POS"}).locator(".." )).toContainText("Cash — Student-linked");
});

test("general cash sale needs no student and stays out of student history",async({page})=>{
  await page.goto("/login");await page.getByRole("tab",{name:"Cafetería"}).click();await page.getByRole("button",{name:/Entrar como cafetería/}).click();await page.getByRole("button",{name:"Ventas en efectivo"}).click();await page.getByRole("button",{name:"Venta general en efectivo"}).click();await expect(page.getByText("Cash — General sale")).toBeVisible();
  const pizza=page.getByRole("article").filter({hasText:"Pizza escolar"});await expect(pizza.getByRole("button",{name:"Añadir al carrito"})).toBeEnabled();await pizza.getByRole("button",{name:"Añadir al carrito"}).click();await page.getByLabel("Efectivo recibido (RD$)").fill("200");await page.getByRole("button",{name:"Confirmar venta"}).click();await expect(page.getByRole("status")).toContainText("Cash — General sale");
  await page.getByRole("button",{name:/Salir/}).click();await page.goto("/login");await page.getByRole("tab",{name:"Estudiante"}).click();await page.getByRole("button",{name:/Entrar como estudiante/}).click();await page.goto("/estudiante/transacciones");await expect(page.getByText("1× Pizza escolar")).toHaveCount(0);
});

test("cafeteria reports filter, export and reconcile cash",async({page})=>{
  await page.goto("/login");await page.getByRole("tab",{name:"Cafetería"}).click();await page.getByRole("button",{name:/Entrar como cafetería/}).click();await page.getByRole("button",{name:"Ventas en efectivo"}).click();await page.getByRole("button",{name:"Venta general en efectivo"}).click();const pasta=page.getByRole("article").filter({hasText:"Pasta con pollo"});await pasta.getByRole("button",{name:"Añadir al carrito"}).click();await page.getByRole("button",{name:"Confirmar venta"}).click();await page.getByRole("button",{name:/Salir/}).click();await adminLogin(page,"admin.cafeteria@demo.pikas.do");await page.goto("/admin/cafeteria/transacciones");await expect(page.getByRole("heading",{name:"Transacciones"})).toBeVisible();await page.getByLabel("Filtrar método").selectOption("cash");await page.getByLabel("Filtrar asociación").selectOption("general_sale");
  await page.getByLabel("Efectivo contado (RD$)").fill("0");await expect(page.getByText("Conteo ingresado").locator("..")).toContainText("RD$0.00");await page.getByLabel("Notas de discrepancia").fill("Conteo ficticio de prueba");
  const download=page.waitForEvent("download");await page.getByRole("button",{name:"Exportar CSV"}).click();await download;await expect(page.getByRole("status")).toContainText("Exportación completada");await expect(page.getByText(/Crédito PIKAS desde efectivo: Desactivado/)).toBeVisible();
});
