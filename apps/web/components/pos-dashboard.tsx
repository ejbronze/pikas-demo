...
export function PosDashboard({demo}:{demo:boolean}){
  ...
  const [generalSale,setGeneralSale]=useState(false);
  ...
  if(!ready)return <main className="grid min-h-screen place-items-center bg-slate-100 p-5"><p role="status" className="card p-5 font-bold">Preparando caja y carrito…</p></main>;
  ...
  const canShop=Boolean(student)||generalSale;
  ...
  const cashReceivedMinor=mode==="cash"&&cashReceived!==""?Math.round(Number(cashReceived)*100):undefined;
  const changeMinor=cashReceivedMinor===undefined?0:Math.max(0,cashReceivedMinor-total);
  const resetSale=()=>{setCart([]);setStudentId(null);setCode("");setCashReceived("");setNotice("")};
  const complete=()=>{if(!student&&!generalSale){setNotice("Verifica un estudiante antes de continuar.");return}const result=checkoutPos(student?.id??null,cart,crypto.randomUUID(),mode,cashReceivedMinor,generalSale);if(!result.ok){setNotice(result.message);return}setCart([]);setCashReceived("");setNotice(`${generalSale?"Cash — General sale":mode==="cash"?"Cash — Student-linked":"Cashless / PIKAS account"} completada: ${money(result.purchase.totalMinor)}.`);setStudentId(null)};
  ...
  return <main className="min-h-screen overflow-x-hidden bg-slate-100 pb-10">
    ...
    <div className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
      ...
      <section className="card p-5"><span className="label">Paso 1</span><h1 className="mt-2 text-2xl font-black">Tipo de venta</h1><div className="mt-4 grid gap-3 sm:grid-cols-2"><button aria-pressed={mode==="student_wallet"} className={`btn-secondary min-h-16 ${mode==="student_wallet"?"ring-2 ring-teal-500":""}`} onClick={()=>{setMode("student_wallet");setGeneralSale(false)}}>Ventas PIKAS</button><button aria-pressed={mode==="cash"} className={`btn-secondary min-h-16 ${mode==="cash"?"ring-2 ring-teal-500":""}`} onClick={()=>setMode("cash")}>Ventas en efectivo</button></div>{mode==="cash"?<div className="mt-3 grid gap-3 sm:grid-cols-2"><button className={`btn-secondary ${!generalSale?"ring-2 ring-teal-500":""}`} onClick={()=>setGeneralSale(false)}>Venta asociada a estudiante</button><button className={`btn-secondary ${generalSale?"ring-2 ring-teal-500":""}`} onClick={()=>{setGeneralSale(true);setStudentId(null)}}>Venta general en efectivo</button></div>:null}<p className="mt-3 font-bold">{generalSale?"Cash — General sale":mode==="cash"?"Cash — Student-linked":"Cashless / PIKAS account"}</p>
        {!student&&!generalSale?<div className="mt-4 max-w-xl"><label className="font-bold" htmlFor="student-code">Código, escaneo/NFC, ID o nombre y fecha de nacimiento</label><div className="relative mt-2"><Search className="absolute left-4 top-3.5 text-slate-400"/><input id="student-code" aria-label="Código estudiantil / NFC" className="field pl-12 uppercase" value={code} onChange={event=>setCode(event.target.value.toUpperCase())} placeholder="PK-10982"/></div><button className="btn mt-3" onClick={verify}>Comprobar estudiante</button>{lookupError?<p id="lookup-error" role="alert" className="mt-3 rounded-xl bg-red-50 p-3 font-bold text-red-900">{lookupError}</p>:null}</div>:null}
        {student?<div className="mt-4 rounded-2xl bg-blue-50 p-4"><h2 className="text-xl font-black">{student.preferredName}</h2><p>{student.grade} · Saldo PIKAS {money(student.balance*100)} · Cobro: {mode==="cash"?"efectivo":"cuenta PIKAS"}</p>{mode==="cash"?<p className="mt-2 font-bold text-amber-900">Esta compra quedará en su historial sin modificar su saldo PIKAS.</p>:null}<div className="mt-2 flex flex-wrap gap-2">{student.allergies.map(value=><span className="chip bg-red-100" key={value}>Alergia: {value}</span>)}{student.blocked.map(value=><span className="chip bg-amber-100" key={value}>Bloqueado: {value}</span>)}</div><button className="btn-secondary mt-3" onClick={()=>setStudentId(null)}>Cambiar estudiante</button></div>:null}
      </section>
      {canShop?<div className="grid gap-5 lg:grid-cols-[1.5fr_.8fr]"><section className="card p-5"><span className="label">Paso 2</span><h2 className="mt-2 text-2xl font-black">Productos</h2><div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]"><input className="field" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar producto"/><div className="flex max-w-full gap-2 overflow-auto pb-1">{categories.map(value=><button className={`chip min-h-11 whitespace-nowrap ${category===value?"bg-slate-950 text-white":""}`} onClick={()=>setCategory(value)} key={value}>{value}</button>)}</div></div><div className="mt-4 grid gap-4 sm:grid-cols-2">{menu.map(item=>{const restriction=restrictionFor(item,student?.allergies??[],student?.blockedProductIds??[],student?.blocked??[]);return <article className="overflow-hidden rounded-2xl border bg-white" key={item.id}><ProductImage src={item.imageUrl} name={item.name}/><div className="p-4"><div className="flex justify-between gap-3"><div><h3 className="font-black">{item.name}</h3><p className="text-sm text-slate-500">{item.category} · {item.description}</p></div><strong>{money(item.priceMinor)}</strong></div><p className="mt-2 text-xs">Ingredientes: {item.ingredients.join(", ")||"No especificados"}</p>{restriction?<p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold">{restriction}</p>:null}<button disabled={Boolean(restriction)} className="btn mt-3 w-full" onClick={()=>add(item.id)}>Añadir al carrito</button></div></article>})}</div>{menu.length===0?<p className="mt-4 rounded-xl bg-slate-50 p-5 text-center">No hay productos para este filtro.</p>:null}</section>
        <aside className="card h-fit p-5"><span className="label">Paso 3</span><h2 className="mt-2 text-2xl font-black">Carrito persistente</h2>{cart.length===0?<div className="mt-4 rounded-xl bg-slate-50 p-6 text-center"><Utensils className="mx-auto"/><p className="mt-2">El carrito está vacío.</p></div>:<div className="mt-3 divide-y">{cart.map(line=>{const item=state.menuItems.find(value=>value.id===line.itemId);return item?<div className="py-3" key={line.itemId}><div className="flex justify-between"><strong>{item.name}</strong><strong>{money(item.priceMinor*line.quantity)}</strong></div><div className="mt-2 flex items-center gap-2"><button className="btn-secondary min-w-11" onClick={()=>setQuantity(item.id,line.quantity-1)}>−</button><span className="font-black">{line.quantity}</span><button className="btn-secondary min-w-11" onClick={()=>setQuantity(item.id,line.quantity+1)}>+</button></div></div>:null})}<div className="flex justify-between py-4 text-xl font-black"><span>Total</span><span>{money(total)}</span></div>{mode==="cash"?<div className="pb-4"><label className="font-bold" htmlFor="cash-received">Efectivo recibido (RD$)</label><input id="cash-received" className="field mt-2" type="number" min={total/100} step="0.01" value={cashReceived} onChange={event=>setCashReceived(event.target.value)} placeholder={String(total/100)}/><p className="mt-2 text-sm font-bold">Cambio: {money(changeMinor)}</p></div>:<p className="pb-4 text-sm">Se descontarán {money(total)} del saldo PIKAS.</p>}<div className="grid gap-2"><button disabled={mode==="cash"&&cashReceivedMinor!==undefined&&cashReceivedMinor<total} className="btn w-full" onClick={complete}>Confirmar venta</button><button className="btn-secondary w-full" onClick={()=>setCart([])}>Cancelar venta</button></div></div>}</aside></div>:null}
      {notice?<section role="status" className="rounded-2xl bg-emerald-50 p-4 text-emerald-900"><h2 className="text-xl font-black">Compra completada</h2><p className="font-bold">{notice}</p><button className="btn mt-3" onClick={resetSale}>Nueva venta</button></section>:null}
      <section className="card p-5"><h2 className="text-2xl font-black">Historial POS</h2>{history.length===0?<div className="mt-4 rounded-xl bg-slate-50 p-6 text-center"><ReceiptText className="mx-auto"/><p>Aún no hay transacciones.</p></div>:<div className="mt-3 divide-y">{history.map(purchase=><button className="flex min-h-16 w-full items-center justify-between gap-4 py-3 text-left" onClick={()=>setSelected(purchase)} key={purchase.id}><div><strong>{purchase.studentName}</strong><p className="text-sm text-slate-500">{purchase.items.map(item=>`${item.quantity}× ${item.name}`).join(", ")} · {purchase.studentAssociation==="general_sale"?"Cash — General sale":purchase.paymentMethod==="cash"?"Cash — Student-linked":"Cashless / PIKAS account"} · {date(purchase.createdAt)}</p></div><strong>{money(purchase.totalMinor)}</strong></button>)}</div>}</section>
    </div>
    {selected?<div className="fixed inset-0 z-50 grid place-items-end bg-slate-950/60 p-0 sm:place-items-center sm:p-4"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-t-3xl bg-white p-6 sm:rounded-3xl"><h2 className="text-2xl font-black">Detalle de transacción</h2><p className="mt-2">{selected.studentAssociation==="general_sale"?"Cash — General sale":selected.paymentMethod==="cash"?"Cash — Student-linked":"Cashless / PIKAS account"} · {selected.studentName}</p><ul className="mt-3 divide-y">{selected.items.map(item=><li className="flex justify-between py-3" key={item.itemId}><span>{item.quantity}× {item.name}</span><strong>{money(item.unitPriceMinor*item.quantity)}</strong></li>)}</ul><button className="btn mt-4 w-full" onClick={()=>setSelected(null)}>Cerrar detalle</button></div></div>:null}
  </main>;
}
